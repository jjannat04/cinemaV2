...
- **TTL Handling**: Redis has built-in TTL expiration; we had to implement a background cleanup task
- **Distributed Locking**: Redis would be better if we ever needed truly distributed locking across multiple database instances
...
- Followed the problem statement's explicit requirements for async payment handling
